...
DIM 브릿지자세중 AS INTEGER
DIM 셔터확인 AS INTEGER
DIM 문확인 AS INTEGER
DIM 브릿지횟수 AS INTEGER
...
적외선거리센서확인:
    ...
    IF 적외선거리값>  80 THEN '50 = 적외선거리값 = 25cm''현승
        문확인 = 0
        셔터확인 = 0
        거리측정 = 0
    	MUSIC "C"
        ...
        IF 셔터확인 = 0 THEN
            ...
            GOSUB 문유무
        ENDIF
        IF 셔터확인 = 0 AND 문확인 = 0 THEN
        	MOVE G6C,100,  30,  80, 100,  46,  96
			WAIT
            DELAY 1000
            MUSIC "E"
            ETX 4800,190 '검정있냐
...
라임이오른발공차기:
    ...
    SPEED 15
    MOVE G6A,115,  73, 150,  90,  90	
    MOVE G6D, 83,  20, 172,  155, 125
    MOVE G6C,50
    MOVE G6B,150
    WAIT
    ...
    SPEED 10
    MOVE G6A,115,  77, 145,  97,  90
    MOVE G6D, 83,  58, 122,  130, 120
    MOVE G6C,100,  40,  80, , , ,
    MOVE G6B,100,  40,  80, , , ,	
    WAIT	

    SPEED 8
    MOVE G6A,115,  77, 145,  95,  95	
    MOVE G6D, 80,  80, 142,  95, 110
    MOVE G6C,100,  40,  80, , , ,
    MOVE G6B,100,  40,  80, , , ,
    WAIT	
...
빨강유무_YES:
    IF R>100 THEN     'R = 1+2+100 = 103 받을거임
        GOSUB 징검다리구간
        redzone=0
    ENDIF
    'ENDIF
